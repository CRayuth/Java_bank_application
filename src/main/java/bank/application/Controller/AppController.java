...
public class AppController implements Initializable {
    ...
    public void getData() {
        ...
        if (cardNum == null || cardNum.isEmpty()) {
            AlertUtil.showAlert("Error", "Card number not found for user.");
            return;
        }
        ...
        if (mainBal != null) {
            mainBal.setText(bal != null ? bal : "0.00");
        }
        
        if (cardBal != null) {
            cardBal.setText(bal != null ? bal : "0.00");
        } else {
        ...
        if (expiryDate != null) {
            expiryDate.setText(date != null ? date : "N/A");
        }
        
        if (cvvNum != null) {
            cvvNum.setText(cvv != null ? cvv : "000");
        }
    ...
    void confirmTopUp() {
        try {
            ...
            // Update balance in the database (only if not in mock mode)
            if (!database.ConnectBankUser.isMockMode()) {
                updateBalanceInDatabase(loggedInUser.getPhonenumber(), newBalance);
            } else {
                // In mock mode, just log the transaction
                System.out.println("Mock Mode: Top-up of $" + topUp + " successful. New balance: $" + newBalance);
            }
    ...
    public void confirmSend() {
        try {
            ...
            if (database.ConnectBankUser.isMockMode()) {
                // Mock transfer - just update sender balance
                ...
                AlertUtil.showAlert("Success", "Mock transfer successful! Sent $" + sendAmountValue + " to card " + recipientCard);
                sendPane.setVisible(false);
                blur2.setVisible(false);
                return;
            }

            // Get recipient balance using card number (database mode)
