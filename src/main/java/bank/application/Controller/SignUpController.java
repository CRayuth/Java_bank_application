...
public class SignUpController implements Initializable {
    ...
    public void registerUser() throws IOException {
        ...
        if (initialPasscode == null || initialPasscode.isEmpty()) {
        ...
        if (ConnectBankUser.isMockMode()) {
            // Mock registration - just create user object without database
            AlertUtil.showAlert("Success", "Mock Registration Successful (Demo Mode)");
            ...
            bank.setBalance(String.valueOf(balance));
            
            // Set the logged-in user after successful registration
            Bank.setLoggedInUser(bank);

            switchToSignIn(null);
            return;
        }

        // Database registration (when not in mock mode)
